...
{
  public:
    int fuel, cap, price, fuelDefault;
    int used_cap = 0, trucksUsed = 0, travelled = 0, p = 0, c = 0;
    ...
    pair<int, int> closestCargo(vector<bool> flag, vector<int> d)
    {
    ...
    int pick(pair<int, int> tmp, vector<bool> &flag)
    {
        travelled += tmp.second;
        fuel -= tmp.second;
        ++p;
        flag[tmp.first] = false;
        return tmp.first;
    }
    ...
    int start(int src, vector<bool> &flag, Graph &g)
    {
        d = g.shortestPath(src);
    ...
    int check(int src, int des, vector<bool> &flag, Graph &g)
    {
        d = g.shortestPath(src);
        tmp = closestCargo(flag, d);
...
int minFuel(int src, int des, vector<bool> &flag, Graph &g)
{
    vector<int> d1;
    d = g.shortestPath(src);
    int weight = 0;
...
int main()
{
    ...
    flag[1] = flag[7] = flag[6] = flag[5] = true;
    ...
    Truck med(fuel * 2, 6, 900);
    Truck large(fuel * 3, 9, 1200);
    Truck xl(fuel * 4, 15, 1500);
    ...
    for (int i = 0; i < 4; i++)
    {
        ...
        cout << "Trucks Used: " << arr[i].trucksUsed << endl;
        cout << "Fuel Used: " << arr[i].travelled << endl;
        cout << "Total Cost: $" << (arr[i].price * arr[i].trucksUsed) + (arr[i].travelled * 50) << endl;
        cout << "-----------------------------------------" << endl;
        cout << endl
             << endl;
    }
